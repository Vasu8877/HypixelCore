                    ...
                    switch ($option) {
                        case 0:
                            if (isset($player::$weathers)) {
                                $player->sendMessage(Utils::BT_MARK . "cYou cannot change it until the server is restarted!");
                                return;
                            }
                            ...
                            $player->sendMessage(Utils::BT_MARK . "bWeather set to rainy!");
                            break;
                        case 1:
                            if (isset($player::$weathers)) {
                                $player->sendMessage(Utils::BT_MARK . "cYou cannot change it until the server is restarted!");
                            ...
                            $packet->eventData = 10000;
                            $player->getNetworkSession()->sendDataPacket($packet);
                            $player->sendMessage(Utils::BT_MARK . "bWeather set to lightning!");
                            break;
                        case 2:
                            $player->getWorld()->setTime(13000);
                            $player->sendMessage(Utils::BT_MARK . "bWeather set to night!");
                            break;
                        case 3:
                            $player->getWorld()->setTime(1000);
                            $player->sendMessage(Utils::BT_MARK . "bWeather set to day!");
                            break;
                    }
                } else {
                    $player->sendMessage(Utils::BT_MARK . "cYou must be on the island to use this feature!");
                }
            }
        );
    }
